...
| 多语言：前端 `common.json` 文案全量覆盖（12 语言） | ☑ 已补齐（`ko/ms/th/vi/ar/sw` 等缺失区块已翻译） |
...
| 日期 | 摘要 |
|------|------|
| 2026-04-08（多语言资源补齐） | 补齐前端多语言资源文件缺失区块：`ko/ms/th/vi/ar/sw` 的 `about/catalog/logistics/footer/ai` 等仍为英文的文案已翻译补全；并补上 `home.consultation`、`home.systems.sectionTitle`、`home.logistics.*` 等关键键，确保切换语言后页面不再大面积回退英文。`npm run lint` 通过。 |
...
| 2026-04-08（部署进行中） | 目标：腾讯云宝塔（IP+HTTP）部署：Nginx(80) 提供 `dist/`，反代 `/api`、`/uploads` 到 Node(8787)；后端用 PM2 守护；Prisma 通过 `db push` 初始化。宝塔已安装 Nginx、Node.js 版本管理器、PM2 管理器，并创建站点目录 `/www/wwwroot/qingtai` 与站点（指向 `/www/wwwroot/qingtai`）。GitHub 仓库 `QT123` 已 push `main`（`origin/main`）。下一步：宝塔 Git 部署拉取到 `/www/wwwroot/qingtai` → `npm ci && npm run build` 生成 `dist/` → 配置环境变量（`DATABASE_URL`/`WEB_ORIGIN`/`API_PORT`/`ADMIN_JWT_SECRET`）→ `prisma generate` + `db push` → PM2 启动 API → Nginx 配置 SPA fallback + 反代。 |
